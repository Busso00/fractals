...
void main() {
    ...
    uvec4 result_f128_a = add_f128(val_a, val_b);
    uvec4 result_f128_s = sub_f128(val_a, val_b);
        ...
        egui::CentralPanel::default().show(ctx, |ui| {
            ...
            for i in 0..INPUT_VALUES.len() { // Corresponds to `value_idx` in GLSL (X-axis)
                for j in 0..INPUT_VALUES.len() { // Corresponds to `value_idy` in GLSL (Y-axis)
                    ...
                    let n_a = input_converted[i] + input_converted[j];
                    
                    let bytes: [u8; 16] = unsafe{
                        mem::transmute::<f128, [u8;16]>(n_a)
                    };

                    let f128_form_a: Vec<String> = bytes
                        .iter()
                        .map(|&b| format!("{:02X}", b))
                        .collect();

                    let n_s = input_converted[i] - input_converted[j];

                    let bytes: [u8; 16] = unsafe{
                        mem::transmute::<f128, [u8;16]>(n_s)
                    };

                    let f128_form_s: Vec<String> = bytes
                        .iter()
                        .map(|&b| format!("{:02X}", b))
                        .collect();
                    ...
                    let byte_strings_a: Vec<String> = current_value_bytes_r
                        .iter()
                        .map(|&b| format!("{:02X}", b))
                        .collect();
                    
                    let byte_strings_s: Vec<String> = current_value_bytes_g
                        .iter()
                        .map(|&b| format!("{:02X}", b))
                        .collect();    
                    ...
                    if true{
                        if f128_form_a != byte_strings_a {                       
                            println!("FAILED TEST [{}][{}] LITTLE ENDIAN: Sum of [{:?}+{:?}] Expected: {:?}, Found: {:?}",
                                i, j,
                        ...
                        if f128_form_s != byte_strings_s {                       
                            println!("FAILED TEST [{}][{}] LITTLE ENDIAN: Sub of [{:?}-{:?}] Expected: {:?}, Found: {:?}",
                                i, j,
                                af128_form, bf128_form,
                                f128_form_s,
                                byte_strings_s
